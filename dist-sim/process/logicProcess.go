...
func CreateLogicProcess(pid int, network []models.ProcessInfo, netFileName string, transitions []models.TransitionMap) *LogicProcess {
	...
	receiveLACh := make(chan int)                  // Canal para recibir LA solicitado a otro proceso
	receiveLAReqCh := make(chan int)               // Canal para recibir solicitud de LA
	sendLookAheadCh := make(chan centralsim.Event) // Canal para enviar LA a otro proceso

	logger := centralsim.CreateLogger(strconv.Itoa(pid))
	simEngine := centralsim.MakeSimulationEngine(lefs, logger, sendEventCh, incomingEventCh, requestLookAheadCh)
